    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled
            ? 'border-b border-white/[0.06] bg-[#0a0a0a]/95 backdrop-blur-xl'
            : 'bg-transparent'
        }`}
      >
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-lg border border-white/[0.08] bg-white/[0.04] text-white transition hover:bg-white/[0.08] md:hidden"
          >
        ...
        {menuOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed right-0 top-0 bottom-0 z-50 flex w-72 flex-col border-l border-white/[0.08] bg-[#0d0d0d] md:hidden"
            >
              {/* Drawer header */}
              <div className="flex items-center justify-between border-b border-white/[0.06] px-6 py-5">
                <img src="/website-logo.png" alt="mikdigital" className="h-7 w-auto brightness-0 invert" />
                <button
                  onClick={() => setMenuOpen(false)}
              ...
              <div className="flex-1 overflow-y-auto px-4 py-6">
                ...
                <div className="mt-8 space-y-3">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex w-full items-center justify-center rounded-xl py-3.5 text-sm font-semibold text-white transition hover:opacity-90"
                    style={{ background: 'linear-gradient(135deg, #7c3aed, #a855f7)' }}
                  >
                    Book Strategy Call
                  </a>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex w-full items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.03] py-3.5 text-sm font-semibold text-white"
                  >
                    WhatsApp
                  </a>
                </div>
                ...
                <div className="mt-8 flex items-center gap-3 border-t border-white/[0.06] pt-6">
                  {socialLinks.map(({ href, icon: Icon, label }) => (
                    <a
                      ...
                      rel="noopener noreferrer"
                      aria-label={label}
                      className="flex h-9 w-9 items-center justify-center rounded-lg border border-white/[0.08] bg-white/[0.03] text-zinc-400 transition hover:text-white"
                    >
                      <Icon size={15} />
                    </a>
                  ))}
